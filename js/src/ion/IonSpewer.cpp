...
ion::CheckLogging()
{
    ...
    if (strstr(env, "help")) {
        ...
        printf(
            ...
            "  linv       Loop inversion\n"
            "  ps         Parameter Specialization\n"
            "  cp         Constant propagation\n"
            "  bce        Bounds check elimination\n"
            "  calls      IonMonkey compiled function calls\n"
    ...
    if (ContainsFlag(env, "linv"))
        EnableChannel(IonSpew_LInv);
    if (ContainsFlag(env, "bcoal"))
        EnableChannel(IonSpew_BCoal);
    if (ContainsFlag(env, "dcec"))
        EnableChannel(IonSpew_DCEC);
    if (ContainsFlag(env, "ps"))
        EnableChannel(IonSpew_PS);
    if (ContainsFlag(env, "cp"))
        EnableChannel(IonSpew_CP);
    if (ContainsFlag(env, "bce"))
        EnableChannel(IonSpew_BCE);
    if (ContainsFlag(env, "calls"))
        EnableChannel(IonSpew_Calls);
    ...
    if (ContainsFlag(env, "asmcode"))
        EnableChannel(IonSpew_ASMCode);
